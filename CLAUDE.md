...
# Invoke (uses RUNTIME_ARN saved to .env)
python3 02_invoke_agentcore_runtime.py
python3 02_invoke_agentcore_runtime.py --dev_name sunshin
```

`agentcore_runtime.py` accepts `dev_name` in the payload, so one runtime serves all developers. `.bedrock_agentcore.yaml` is auto-generated by the starter toolkit on first launch.
